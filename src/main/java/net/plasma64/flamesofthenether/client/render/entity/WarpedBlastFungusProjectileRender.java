...
public class WarpedBlastFungusProjectileRender extends EntityRenderer<WarpedBlastFungusProjectile> {
    private final BlockRenderDispatcher blockRenderer;
    public WarpedBlastFungusProjectileRender(EntityRendererProvider.Context pContext) {
        super(pContext);
    ...
    @Override
    public void render(WarpedBlastFungusProjectile pEntity, float pEntityYaw, float pPartialTick, PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight) {
        pPoseStack.pushPose();
        ...
        pPoseStack.mulPose(Axis.YP.rotationDegrees(90.0F));
        TntMinecartRenderer.renderWhiteSolidBlock(this.blockRenderer, FOTNBlockRegistry.WARPED_BLAST_FUNGUS_BLOCK.get().defaultBlockState(), pPoseStack, pBuffer, pPackedLight, fuse / 5 % 2 == 0);
        pPoseStack.popPose();
    }

    public static void renderModel(PoseStack.Pose pPose, VertexConsumer pConsumer, @Nullable BlockState pState, BakedModel pModel, float pRed, float pGreen, float pBlue, int pPackedLight, int pPackedOverlay, ModelData modelData, net.minecraft.client.renderer.RenderType renderType) {
    ...
    @Override
    public ResourceLocation getTextureLocation(WarpedBlastFungusProjectile pEntity) {
